...
## 2026-05-15 下午 — FEP 完成 / 2×2 矩阵部署 / ΔG 分析

### FEP 结果（D-Ala2 ↔ L-Ala2 手性翻转）

11 个 λ 窗口各 5 ns，GROMACS 2026 dual-topology，~100k 原子体系。
...
**结论**：ΔG(D-Ala2 → L-Ala2) = +0.83 kJ/mol，小于 kT (2.57 kJ/mol)。两种手性形式对 DPP-IV 的结合亲和力没有显著差异。D-Ala2 的抑制作用来自**非生产性结合姿态**（0% productive pose），而非结合更强。

### 2×2 矩阵完整部署

| | 全长 (1-29) | 短肽 (1-10) |
|---|---|---|
| **D-Ala2** | GPU 0, 130.6 ns | GPU 3, 143.1 ns |
| **L-Ala** | GPU 1, 44.8 ns → dt=0.002 重启 | GPU 2, 0 ns → 新启动 |

- GPU 1：天然 GHRH 从 dt=0.001 切换到 dt=0.002（44.8 ns checkpoint 提取，新 tpr）
- GPU 2：L-Ala 短肽由自动监控脚本 `launch_LAla_when_fep_done.sh` 在 FEP 完成后触发
- 之前 stop 了 D-Ala2 短肽（139 ns）腾 GPU 跑 FEP batch 4，FEP 完成后已续跑
- GPU 3 的 D-Ala2 短肽从 checkpoint（143.1 ns）续跑

### 科学图景（修正标签后）

1. **结合亲和力**：D-Ala2 ≈ L-Ala2（FEP ΔG ≈ 0）
2. **催化几何**：D-Ala2 保持在口袋近端（3.15 Å, 93.8% <4Å）但攻击角错误（0% productive）
3. **L-Ala 解离**：天然底物在无 QM 的 MD 中从口袋漂移（6.31 Å），因为无法模拟酶切反应
4. **抑制机制**：D-Ala2 是竞争性抑制剂——结合但不被切割，锁死在非生产构象

### 待完成

- 所有 MD 跑到 200 ns
- 四系统统一分析（RMSF/SASA/氢键/盐桥）
- MM-PBSA 结合自由能（补充 FEP 结果）

---

## 2026-05-16 — D-Ala2 两条轨迹即将完成 / L-Ala 性能优化

### 当前 MD 状态（2026-05-16 上午）

| GPU | 系统 | 进度 | 性能 | ETA |
|-----|------|------|------|-----|
| 0 | CJC-1295 (D-Ala2, 1-29) | 184.5 ns | 74 ns/d | **今天** |
| 3 | D-Ala2 短肽 (1-10) | 184.1 ns | 69 ns/d | **今天** |
| 2 | L-Ala 短肽 (1-10) v2 | 25.9 ns | 60 ns/d | 5/19 |
| 1 | 天然 GHRH (L-Ala, 1-29) | 53.7 ns | 19 ns/d | 5/24 |

### dt=0.002 加速实验（失败）

- L-Ala 全长从 44.8 ns checkpoint 提取帧，换 dt=0.002 重启
- GPU 利用率仅 13%，性能 ~9 ns/day（原 32 ns/day）
- 根因：从 checkpoint 提取帧丢失速度信息，系统重新热化
- 回退 checkpoint 续跑，性能稳定在 19 ns/day（略低于原来的 25-32）
- **经验**：不能通过提取帧+新建 tpr 来改 dt；正确做法是用 `gmx convert-tpr`

### L-Ala 短肽性能优化

- 原始性能：25 ns/day（nstlist=10, ntmp=8）
- 激进 benchmark：试了 ntmp=4/6/8/12/16, dlb=yes/no, dt=0.004, rc=0.9, nstlist=40/50, ntmpi=2
- dt=0.004 crash（CUDA illegal memory access，需 HMR 未做）
- 最优配置：rc=0.9, nstlist=50, rlist=1.1, ntmp=6 → **60 ns/day**（+140%）
- 两短肽体系（97k atom）性能上限约 60-70 ns/day，受限于 PME FFT 网格（96³）的相对开销

### 下一步

- GPU0/3 今天完成 → 可提前开始 D-Ala2 全长 vs 短肽对比分析
- GPU2 L-Ala 短肽 3 天后完成 → 补齐 2×2 矩阵
- GPU1 是瓶颈（~8 天），但 dt=0.001 是唯一稳定配置
...
## 2026-05-16 — GPU1 天然 GHRH 性能优化（重大突破）

### 问题
天然 GHRH (L-Ala, 1-29) 在 GPU 1 上性能仅 **19 ns/day**，GPU 利用率仅 53%，成为整个 2×2 矩阵的瓶颈。

### 诊断
- CJC-1295 (GPU 0) 同参数同硬件：**74 ns/day**，GPU 94%
- 天然 GHRH (GPU 1) 同参数同硬件：**19 ns/day**，GPU 53%
- 唯一区别：天然 GHRH 用了 `-pin on -ntomp 16`，CJC-1295 没有 `-pin`

**根因**：`-pin on` 将 16 个 OpenMP 线程固定到特定 CPU 核心。4 个 GPU 上的 mdrun 进程同时运行时，固定线程与其他进程的线程发生 CPU 核心竞争，导致 GPU 大量等待 CPU，利用率暴跌。

### 优化措施
1. **移除 `-pin on`**：允许线程在所有 128 逻辑核心上自由调度
2. **减少 `-ntomp` 从 16 → 8**：降低 OpenMP 同步开销和核心竞争

### 结果
| 指标 | 优化前 | 优化后 | 变化 |
|------|--------|--------|------|
| 性能 | **19 ns/day** | **~33 ns/day** | **+73%** |
| GPU 利用率 | 53% | 93% | +40 pp |
| ETA (200 ns) | 7.6 天 | 4.4 天 | -3.2 天 |
| 每步耗时 | ~4.6 ms | ~2.6 ms | -43% |

### 已淘汰的方案
- dt=0.002 + gen-vel=yes：性能 22.5 ns/day，但系统不稳定（LINCS 无法收敛，大量 CPU 时间）
- dt=0.002 + checkpoint 速度 (grompp -t)：性能 20.9 ns/day，提升太小不值得丢失 54 ns 进度
- dt=0.001 改 nstlist/rc/pcoupl：GROMACS 不允许从 checkpoint 续跑时改变核心算法参数

### 教训
- **`-pin on` 在多 GPU 并行场景下是性能陷阱**：当多个 mdrun 同时运行时，固定线程会导致严重竞争
- **ntomp 不是越多越好**：对于 GPU-bound 的 MD，8 线程通常足够，16 线程反而增加同步开销
...
#### 1.2 ns 生产运行结果（2026-05-16 完成）
- **温度**：310.35 ± 0.46 K ✅ 与目标 310K 完美匹配
- **密度**：995.53 ± 0.83 kg/m³
- **体积**：3301.93 ± 2.74 nm³
- **势能**：-4,416,247 ± 2,075 kJ/mol
- **总能量**：-3,569,252 ± 2,494 kJ/mol
- **性能**：43.2 ± 0.1 ns/day，总耗时 ~40 分钟（含 equil）

#### 结构分析
- **RMSD**：2.364 ± 0.763 Å（范围 0–3.765 Å）
  - 持续漂移表明系统仍在弛豫，需 >10 ns 收敛
- **RMSF (CA)**：0.853 ± 0.248 Å，最高残基 206 = 1.842 Å
- **回转半径 Rg**：38.875 ± 0.096 Å（范围 38.680–39.122 Å）

#### OpenMM vs GROMACS 对比
| 指标 | OpenMM (1.2 ns) | GROMACS (145 ns) | 结论 |
|------|----------------|-----------------|------|
| 温度 | 310.35 ± 0.46 K | ~310 K | ✅ 完美匹配 |
| 密度 | 995.5 ± 0.8 kg/m³ | ~1016 kg/m³ | ~2% 偏低（barostat 差异）|
| 速度 | 43.2 ns/day | ~33 ns/day | ✅ OpenMM 快 30% |

#### 稳定性验证
- 最小化：1000 步，PE = -5,510,491 kJ/mol
- NVT 100 ps：164.8s，PE = -4,416,524 kJ/mol，速度 52.4 ns/day ✅
- NPT 100 ps：194.8s，PE = -4,417,325 kJ/mol，速度 44.4 ns/day ✅
- 生产 1.2 ns：无 NaN，温度稳定

#### 文件
- `workspace/step3/openmm_production.py` — 完整生产脚本
- `workspace/step3/OPENMM_REPLICA.md` — 技术文档
- `workspace/step3/openmm_analysis_summary.txt` — 分析摘要
- `workspace/step3/openmm_analysis.png` — 分析图表（6-panel）
...
| 系统 | GPU | 进度 | 性能 | 状态 |
|------|-----|------|------|------|
| **D-Ala2 全长** | 1 | **185.2 / 200 ns** | ~33 ns/day | 🔄 预计 10–12 小时完成 |
| **L-Ala 短肽** | 2 | **200 / 200 ns ✅** | **161.73 ns/day** | ✅ **已完成**（04:20）|
| **L-Ala 全长** | 3 | **~41.2 / 200 ns** | ~33 ns/day | 🔄 正常 |

#### L-Ala 短肽完成详情
- **完成时间**：2026-05-17 04:20:52
- **总步数**：100,000,000（200 ns，dt = 2 fs）
- **Wall time**：106,844 s（~29.7 小时）
- **性能**：161.73 ns/day（小体系优势）
- **温度**：~310 K 稳定
- **输出**：`LAla_short_v2.xtc` (8.5G), `LAla_short_v2.log`

#### GPU 利用率
- GPU 0：空闲（OpenMM 已完成）
- GPU 1：95%（D-Ala2 全长）
- GPU 2：空闲（L-Ala 短肽已完成）
- GPU 3：76%（L-Ala 全长）

### 下一步
1. 等待 D-Ala2 全长完成（预计 5/17 晚间）
2. 启动四系统统一分析（RMSF/SASA/氢键/盐桥）
3. L-Ala 全长继续跑至 200 ns
